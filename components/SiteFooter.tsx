...
const FALLBACK_LOCALITIES = ['NH-48 Entry Point', 'Delhi Road', 'Sadar Bazar', 'Bus Stand', 'Subhash Nagar', 'Model Town']
...
export default function SiteFooter({ localities }: { localities?: string[] }) {
  const locationLinks = localities && localities.length > 0 ? localities.slice(0, 6) : FALLBACK_LOCALITIES
  const waUrl = `https://wa.me/${WA}?text=${encodeURIComponent("Hi, I'd like to enquire about hoarding advertising in Rewari.")}`
          ...
          <div>
            ...
            <ul className="space-y-2 list-none m-0 p-0">
              {[['All Rewari Locations', '/city/rewari'], ['Unipoles', '/type/unipole'], ['Gantries', '/type/gantry'], ['Hoardings', '/type/hoarding'], ['LED Displays', '/type/led-display'], ['Bus Shelters', '/type/bus-shelter']].map(([label, href]) => (
                <li key={href}>
                  <Link href={href} className="font-condensed font-medium text-[15px] text-[#555] uppercase tracking-[0.04em] hover:text-white transition-colors no-underline">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2 list-none m-0 p-0">
              {locationLinks.map(loc => (
                <li key={loc}>
                  <Link href={`/locality/${slugify(loc)}`} className="font-condensed font-medium text-[15px] text-[#555] uppercase tracking-[0.04em] hover:text-white transition-colors no-underline">
                    {loc}
                  </Link>
                </li>
              ))}
